...
u1old = 0;
u2old = 0;
out = sim('adaptation_part_a.slx',tfinal);
t = [t;out.tout];
xm = [xm;out.xm];
x = [x;out.x];
eps = [eps;out.epsilon];
...
nepsilon1 = sqrt(trapz(out.tout,sum((out.epsilon)'.^2)));

x0 = [1,1,1];
u1old = 0;
u2old = 0;
out = sim('adaptation_part_b.slx',tfinal);
t = [t;tfinal+out.tout];
xm = [xm;out.xm];
x = [x;out.x];
eps = [eps;out.epsilon];
...
nepsilon2 = sqrt(trapz(out.tout,sum((out.epsilon)'.^2)));

x0 = [1,1,1];
u1old = 0;
u2old = 0;
out = sim('adaptation_part_c.slx',tfinal);
...
nepsilon3 = sqrt(trapz(out.tout,sum((out.epsilon)'.^2)));

x0 = [1,1,1];
u1old = 0;
u2old = 0;
out = sim('adaptation_part_d.slx',tfinal);
...
nepsilon4 = sqrt(trapz(out.tout,sum((out.epsilon)'.^2)));
